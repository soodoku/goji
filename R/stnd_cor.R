...
#' 
#' @param item response to the knowledge item. We assume that the domain of values
...
#' @param n_options total number of options; numeric; default is 4
#' 
#' @export
#' @examples
#' x <- c(NA, 1, 0, 1, 0, 0); mean(x, na.rm = T); stnd_cor(x, 4)

stnd_cor <- function(item, n_options = 4) {

  total_correct <- sum(item == 1, na.rm = TRUE)
  total_incorrect <- sum(item == 0, na.rm = TRUE)

  est_guessed <- total_incorrect/(n_options - 1)
  adjusted <- (total_correct - est_guessed)/ length(item)
  adjusted
}
